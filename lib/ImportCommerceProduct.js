...
class ImportCommerceProduct extends ImportClass {
  ...
  */
  constructor(req, res) {
    ...
    this.cmsWorkspaceId = req.body.options.cms_workspace
    this.contentVersionResponse = ''
    this.defaultEntitlementPolicyId = req.body.options.entitlement
    this.importApi = ''
    this.mapping = req.body.mapping
    this.options = req.body.options
    this.pimProductId = req.body.pimProductId
    this.productCatalogId = req.body.options.catalog
    this.pricebookObj = {}
    this.webstoreId = req.body.options.webstore

    this.pricebookObj[req.body.options.pricebook_name] = req.body.options.pricebook_id
  ...
  async createVariantParent() {
    ...
    console.log('starting building maps')
    await cpv.buildMaps()
    console.log('end building maps')
  ...
  async sendToProductApi() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const importConfigStr = JSON.stringify(new ImportConfiguration(
        ...
        this.pricebookObj, // pricebookIds
