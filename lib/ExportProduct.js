...
class ExportProduct {
   ...
   */
  constructor(req) {
    const { hostUrl, options, recordIds, sessionId, variantValueIds } =
      req.body;
    ...
    this.helper = propelConnect.newHelper(
      this.connection,
      {},
      this.namespace,
      {}
  ...
  async populateProductMap() {
    ...
    const pimProductDeep = new PimProductDeep(
      this.helper,
      this.log,
      querySaveProductIds
    );
  ...
  buildExportObject() {
    ...
    for (const key of this.productMap.keys()) {
      ...
      tmpObject['Record_ID'] = key;

      this.productMap.get(key).forEach(attribute => {
        tmpObject[attribute['Attribute_Label__r'].Label__c] =
          attribute['Value__c'];
      });
  ...
  async sendToChatter() {
    ...
    this.chatter.postToChatterWithFile(this.fileName, '', false, true, {});
  }
