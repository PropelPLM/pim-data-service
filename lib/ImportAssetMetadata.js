    ...
    for (let x = 0; x < this.propelParser.nodes.length; x++) {
      ...
      if (
        ...
        this.labelNames.forEach((label) => {
          this.createTempAttributeValueObjects(label, assetSObjectId, this.propelParser.nodes[x][label], tempAttrValuesToUpdate, tempAttrValuesToInsert);
        ...
        results = await this.connection.update(this.helper.namespace('Attribute_Value__c'), tempAttrValuesToUpdate);
        this.log.addToLogs(results, this.helper.namespace('Attribute_Value__c'));

        results = await this.connection.insert(this.helper.namespace('Attribute_Value__c'), tempAttrValuesToInsert);
        this.log.addToLogs(results, this.helper.namespace('Attribute_Value__c'));
      }
    }
  }
