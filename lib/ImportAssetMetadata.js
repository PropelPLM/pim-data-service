...
class ImportAssetMetadata extends ImportClass {
  ...
  async updateExistingAssets() {
    for (let x = 0; x < this.propelParser.nodes.length; x++) {
      ...
      if (
        ...
        await this.updateAssetCategory(assetSObjectId, this.propelParser.nodes[x].category_id);
        
        // let tempAttrValuesToUpdate = [];
        // let tempAttrValuesToInsert = [];
        let sortedTempAttrVals;
        let results;
        // let tempAttrVal;
        this.labelNames.forEach((label) => {
          sortedTempAttrVals = this.createTempAttributeValueObjects(label.slice(1, -1), assetSObjectId, this.propelParser.nodes[x][label], tempAttrValuesToUpdate, tempAttrValuesToInsert);
        ...
        results = await this.connection.update(
          this.helper.namespace('Attribute_Value__c'), sortedTempAttrVals.tempAttrValuesToUpdate
        );
        this.log.addToLogs(results, this.helper.namespace('Attribute_Value__c'));

        results = await this.connection.insert(
          this.helper.namespace('Attribute_Value__c'), sortedTempAttrVals.tempAttrValuesToInsert
        );
        this.log.addToLogs(results, this.helper.namespace('Attribute_Value__c'));
      }
    }
  ...
  async updateAssetCategory(assetSObjectId, categoryId) {
    let tempAsset = new Object();
    tempAsset['Id'] = assetSObjectId;
    tempAsset[this.helper.namespace('Category__c')] = this.categoryMap.get(categoryId);

    let results = await this.connection.update(this.helper.namespace('Digital_Asset__c'), new Array(tempAsset));
    this.log.addToLogs(results, this.helper.namespace('Digital_Asset__c'));
  }

  createTempAttributeValueObjects(label, assetSObjectId, value, tempAttrValuesToUpdate, tempAttrValuesToInsert) {
    let sortedTempAttrVals = new Object();
    let tempAttrVal;
    if (this.assetLabelValueMap.has(assetSObjectId) && this.assetLabelValueMap.get(assetSObjectId).has(label)) {
      // update existing attribute value
      tempAttrVal = new Object();
      ...
      sortedTempAttrVals['tempAttrValuesToUpdate'].push(tempAttrVal);
    } else {
      // create new attribute value
      tempAttrVal = new Object();
      tempAttrVal['Digital_Asset__c'] = assetSObjectId;
      tempAttrVal['Attribute_Label__c'] = this.labelNameIdMap.get(label);
      tempAttrVal['Value__c'] = value;
      sortedTempAttrVals['tempAttrValuesToInsert'].push(tempAttrVal);
    }
    return sortedTempAttrVals;
  }
