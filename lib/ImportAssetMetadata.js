...
class ImportAssetMetadata extends ImportClass {
  ...
  async updateExistingAssets() {
    ...
    for (let x = 0; x < this.propelParser.nodes.length; x++) {
      ...
      if (node.digital_asset_id && this.existingAssetMap.has(node.digital_asset_id)) {
        tempAsset = new Object()
        tempAsset[this.helper.namespace('Category__c')] = this.categoryMap.get(this.propelParser.nodes[x].category_id);
