    ...
    for (let x = 0; x < this.propelParser.nodes.length; x++) {
      ...
      if (
        ...
        this.labelNames.forEach((label) => {
          if (this.assetAttributeValueMap.has(assetSObjectId) && this.assetAttributeValueMap.get(assetSObjectId).has(label)) {
            console.log('this.propelParser.nodes[x][label]: ', this.propelParser.nodes[x][label])
            this.assetAttributeValueMap.get(assetSObjectId).get(label).Value =  this.propelParser.nodes[x][label];
          }
        });
      }
    }
