...
class ImportProduct extends ImportClass {
        ...
        if (this.productMap.has(node.product_id)) {
          Object.keys(node).forEach(key => {
            ...
            if (!this.attributeExemptList.includes(key) && this.attributeLabelMap.has(key) && node[key]) {
              ...
              console.log('product node: ', node)
              console.log('this.attributeLabelMap.get(key): ', this.attributeLabelMap.get(key))
  ...
  async processVariants() {
      ...
      this.nodeMap.get('0').forEach(node => {
        ...
        Object.keys(node).forEach(key => {
          if (key.startsWith('variant_') && node[key]) {
            console.log('processVariants node[key]: ', node[key])
            console.log('processVariants key: ', key)
            console.log('processVariants node: ', node)
            ...
            console.log('processVariants this.productMap: ', this.productMap)
            console.log('processVariants this.productMap.get(node.product_id): ', this.productMap.get(node.product_id))
    ...
    console.log('processVariants insertVariants: ', insertVariants)
  ...
  async processVariantValues() {
    ...
    for (const mapKey of this.nodeMap.keys()) {
      if (mapKey !== '0') {
        console.log('processVariantValues mapKey: ', mapKey)
        this.nodeMap.get(mapKey).forEach(node => {

          console.log('processVariantValues node: ', node)
          console.log('processVariantValues this.variantValueMap: ', this.variantValueMap)
          if (!this.variantValueMap.has(node.product_id)) {
            console.log('processVariantValues  node 2: ', node)
            Object.keys(node).forEach(key => {
              if (key.startsWith('variant_') && node[key]) {
                ...
                console.log('processVariantValues tmpVariantValue1: ', tmpVariantValue)
                console.log('processVariantValues node: ', node)
                console.log('processVariantValues key: ', key)
                
                if (this.productHasVariantMap.has(node.highest_level_product_id)) {
                  console.log('processVariantValues this.productHasVariantMap: ', this.productHasVariantMap)
                  console.log('processVariantValues this.productHasVariantMap.get(node.highest_level_product_id): ', this.productHasVariantMap.get(node.highest_level_product_id))
                  this.productHasVariantMap.get(node.highest_level_product_id).forEach(variant => {
                    console.log('processVariantValues variant: ', variant)
                  ...
                  console.log('processVariantValues tmpVariantValue2: ', tmpVariantValue)
