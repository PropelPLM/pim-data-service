...
class ImportCategory {
  ...
  constructor(req, res) {
    ...
    this.propelLog = propelConnect.newLog(this.connection, this.orgId)
  ...
  async start() {
    ...
    // finish up
    await this.propelLog.sendReport()
  }
  ...
  async populateCategoryMap() {
    ...
    const pimCategory = new PimCategory(this.helper, this.propelLog)
  ...
  async processLineByLine() {
    ...
    for (let x = 0; x < this.nodes.length; x++) {
      ...
      this.propelLog.addToLogs(results, this.helper.namespace('Category__c'))
