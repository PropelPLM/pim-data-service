...
class ImportAttributeLabel extends ImportClass {
  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      tmpAttributeLabel[this.helper.namespace('Is_Required__c')] = (node.required && node.required.toString().toUpperCase() === 'TRUE') ? true : false;
      console.log('node.required: ', node.required);
      console.log('typeof(node.required): ', typeof(node.required));
