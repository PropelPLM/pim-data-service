...
class ImportAttributeLabel extends ImportClass {
  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      let minValue = (node.type.toUpperCase() === NUMBER_TYPE && node.min_value) ? parseInt(node.min_value, 10) : null;
      let maxValue = (node.type.toUpperCase() === NUMBER_TYPE && node.max_value) ? parseInt(node.max_value, 10) : null;
      console.log('minValue1: ', minValue);
      console.log('maxValue1: ', maxValue);
      if (minValue > maxValue) {
        minValue = null;
        maxValue = null;
        console.log('minValue2: ', minValue);
        console.log('maxValue2: ', maxValue);
      }
      ...
      tmpAttributeLabel[this.helper.namespace('Min_Value__c')] = minValue;
      tmpAttributeLabel[this.helper.namespace('Max_Value__c')] = maxValue;
