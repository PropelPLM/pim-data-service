...
class ImportAttributeLabel extends ImportClass {
  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      console.log('node.type === number: ', node.type === 'number')
      console.log('node.min_value: ', node.min_value)
      console.log('node.max_value: ', node.max_value)
      if (node.type === 'number' && node.min_value) {
        console.log('min value added')
        tmpAttributeLabel[this.helper.namespace('Min_Value__c')] = node.min_value
      }
      if (node.type === 'number' && node.max_value) {
        console.log('max value added')
        tmpAttributeLabel[this.helper.namespace('Max_Value__c')] = node.max_value
      }
