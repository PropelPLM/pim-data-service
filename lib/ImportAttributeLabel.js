  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      if (node.type === 'number' && node.min_value) {
        tmpAttributeLabel[this.helper.namespace('Min_Value__c')] = parseInt(node.min_value, 10)
      }
      if (node.type === 'number' && node.max_value) {
        tmpAttributeLabel[this.helper.namespace('Max_Value__c')] = parseInt(node.max_value, 10)
      }
