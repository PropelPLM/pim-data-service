...
class ImportAttributeLabel extends ImportClass {
  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      tmpAttributeLabel[this.helper.namespace('Min_Value__c')] = (node.type === 'number' && node.min_value) ? parseInt(node.min_value, 10) : null;
      tmpAttributeLabel[this.helper.namespace('Max_Value__c')] = (node.type === 'number' && node.max_value) ? parseInt(node.max_value, 10) : null;
      console.log('tmpAttributeLabel: ', tmpAttributeLabel)
