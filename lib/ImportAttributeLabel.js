...
class ImportAttributeLabel extends ImportClass {
  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      let minValue = (node.type.toUpperCase() === NUMBER_TYPE && node.min_value) ? Number(node.min_value) : null;
      let maxValue = (node.type.toUpperCase() === NUMBER_TYPE && node.max_value) ? Number(node.max_value) : null;
