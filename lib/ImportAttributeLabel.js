...
class ImportAttributeLabel extends ImportClass {
  ...
  async processLineByLine() {
    ...
    this.propelParser.nodes.forEach((node) => {
      ...
      const minValue = (node.type.toUpperCase() === NUMBER_TYPE && node.min_value) ? parseInt(node.min_value, 10) : null;
      const maxValue = (node.type.toUpperCase() === NUMBER_TYPE && node.max_value) ? parseInt(node.max_value, 10) : null;
