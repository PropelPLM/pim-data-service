...
class ImportAttributeTab {
  ...
  async processLineByLine() {
    ...
    try {
      ...
      this.log.addToLogs(JSON.stringify(err), 'from ImportAttributeTab.processLineByLine')
    }
  }
  ...
  async addLinks() {
    ...
    try {
      this.propelParser.nodes.forEach((node) => {
        ...
        node.primary_category_associations.split(';').forEach((cat) => {
      ...
      this.log.addToLogs(JSON.stringify(err), 'from ImportAttributeTab.addLinks')
    }
  }
}
